...
class UNIQUE_API Node : public Object
{
	uRTTI(Node, Object)
public:
    /// Construct.
    ...
    Node* CreateChild(const StringID& name = String::EMPTY, unsigned id = 0, bool temporary = false);
    /// Create a temporary child scene node (with specified ID if provided).
    Node* CreateTemporaryChild(const StringID& name = String::EMPTY, unsigned id = 0);
    ...
    Component* CreateComponent(StringID type, unsigned id = 0);
    /// Create a component to this node if it does not exist already.
    Component* GetOrCreateComponent(StringID type, unsigned id = 0);
    ...
    /// Set a user variable.
    //void SetVar(StringID key, const Variant& value);
    ...
    /// Return name.
    const StringID& GetName() const { return name_; }
    ...
    const Vector<SPtr<Node> >& GetChildren() const { return children_; }
    ...
    void GetChildrenWithComponent(PODVector<Node*>& dest, StringID type, bool recursive = false) const;
    /// Return child scene nodes with a specific component.
    PODVector<Node*> GetChildrenWithComponent(StringID type, bool recursive = false) const;
    ...
    Node* GetChild(StringID nameHash, bool recursive = false) const;

    /// Return number of components.
    unsigned GetNumComponents() const { return (unsigned)components_.size(); }
	
    /// Return all components.
    const Vector<SPtr<Component> >& GetComponents() const { return components_; }

    /// Return all components of type. Optionally recursive.
    void GetComponents(PODVector<Component*>& dest, StringID type, bool recursive = false) const;
    /// Return component by type. If there are several, returns the first.
    Component* GetComponent(StringID type, bool recursive = false) const;
    /// Return component in parent node. If there are several, returns the first. May optional traverse up to the root node.
    Component* GetParentComponent(StringID type, bool fullTraversal = false) const;
...
private:
    ...
    void RemoveChild(Vector<SPtr<Node> >::iterator i);
    /// Return child nodes recursively.
    void GetChildrenRecursive(PODVector<Node*>& dest) const;
    /// Return child nodes with a specific component recursively.
    void GetChildrenWithComponentRecursive(PODVector<Node*>& dest, StringID type) const;
    ...
    void GetComponentsRecursive(PODVector<Component*>& dest, StringID type) const;
    /// Clone node recursively.
    Node* CloneRecursive(Node* parent, SceneResolver& resolver);
    /// Remove a component from this node with the specified iterator.
    void RemoveComponent(Vector<SPtr<Component> >::iterator i);
...
private:
    ...
    /// Child scene nodes.
    Vector<SPtr<Node> > children_;
    /// Node listeners.
    Vector<WPtr<Component> > listeners_;
	/// Name.
	StringID name_;
	/// Tag strings.
	StringVector tags_;
