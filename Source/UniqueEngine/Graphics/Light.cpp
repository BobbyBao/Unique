...
void Light::ProcessRayQuery(const RayOctreeQuery& query, PODVector<RayQueryResult>& results)
{
    ...
    case RAY_TRIANGLE_UV:
        UNIQUE_LOGWARNING("RAY_TRIANGLE_UV query level is not supported for Light component");
        return;
    }
...
Frustum Light::GetFrustum() const
{
    ...
    Matrix3x4 frustumTransform(node_ ? Matrix3x4(node_->GetWorldPosition(), node_->GetWorldRotation(), Vector3::ONE) :
                               Matrix3x4::IDENTITY);
    Frustum ret;
...
const Matrix3x4& Light::GetVolumeTransform(Camera* camera)
{
    ...
    case LIGHT_POINT:
        volumeTransform_ = Matrix3x4(node_->GetWorldPosition(), Quaternion::IDENTITY, Vector3(range_, range_, range_));
        break;
    }
...
{
    return GetResourceRef(rampTexture_, Texture::GetTypeStatic());
}

ResourceRef Light::GetShapeTextureAttr() const
{
	return GetResourceRef(shapeTexture_, Texture::GetTypeStatic());
}
