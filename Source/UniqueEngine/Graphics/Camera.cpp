...
{
	uFactory("Scene")
	uAccessor("Near Clip", GetNearClip, SetNearClip, DEFAULT_NEARCLIP)
	uAccessor("Far Clip", GetFarClip, SetFarClip, DEFAULT_FARCLIP)
	uAccessor("FOV", GetFov, SetFov, DEFAULT_CAMERA_FOV)
	uAccessor("Aspect Ratio", GetAspectRatio, SetAspectRatioInternal, 1.0f)
	uAttribute("Fill Mode", fillMode_, FILL_MODE_SOLID)
	uAttribute("Auto Aspect Ratio", autoAspectRatio_, true)
	uAccessor("Orthographic", IsOrthographic, SetOrthographic, false)
	uAccessor("Orthographic Size", GetOrthoSize, SetOrthoSizeAttr, DEFAULT_ORTHOSIZE)
	uAccessor("Zoom", GetZoom, SetZoom, 1.0f)
	uAccessor("LOD Bias", GetLodBias, SetLodBias, 1.0f)
	uAttribute("View Mask", viewMask_, DEFAULT_VIEWMASK);
	uAttribute("View Override Flags", viewOverrideFlags_, VO_NONE)
	uAccessor("Projection Offset", GetProjectionOffset, SetProjectionOffset, Vector2::ZERO)
	uMixedAccessor("Reflection Plane", GetReflectionPlaneAttr, SetReflectionPlaneAttr, Vector4(0.0f, 1.0f, 0.0f, 0.0f))
	uMixedAccessor("Clip Plane", GetClipPlaneAttr, SetClipPlaneAttr, Vector4(0.0f, 1.0f, 0.0f, 0.0f))
	uAccessor("Use Reflection", GetUseReflection, SetUseReflection, false)
	uAccessor("Use Clipping", GetUseClipping, SetUseClipping, false)
