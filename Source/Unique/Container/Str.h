...
public:
    ...
    const_iterator End() const { return const_iterator(buffer_ + length_); }
...
{
	template<>
	struct hash<Unique::String>
	{
		size_t operator()(const Unique::String& _Keyval) const
		{
	...
	template<>
	struct hash<Unique::WString>
	{
		size_t operator()(const Unique::WString& _Keyval) const
		{
			unsigned hash = 0;
			const wchar_t* ptr = _Keyval.CString();
			while (*ptr)
			{
				hash = *ptr + (hash << 6) + (hash << 16) - hash;
				++ptr;
