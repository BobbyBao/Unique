...
public:
    ...
    template <class U> SPtr(const SPtr<U>& rhs) :
        ptr_(rhs.ptr_)
    {
        AddRef();
    }
    ...
    template <class U> SPtr<T>& operator =(const SPtr<U>& rhs)
    {
        if (ptr_ == rhs.ptr_)
            return *this;
    ...
    template <class U> bool operator <(const SPtr<U>& rhs) const { return ptr_ < rhs.ptr_; }
    ...
    template <class U> void StaticCast(const SPtr<U>& rhs)
    {
        ReleaseRef();
    ...
    template <class U> void DynamicCast(const SPtr<U>& rhs)
    {
        ReleaseRef();
...
private:
    template <class U> friend class SPtr;
...
template <class T, class U> SPtr<T> StaticCast(const SPtr<U>& ptr)
{
    SPtr<T> ret;
    ret.StaticCast(ptr);
    return ret;
...
template <class T, class U> SPtr<T> DynamicCast(const SPtr<U>& ptr)
{
    SPtr<T> ret;
    ret.DynamicCast(ptr);
    return ret;
