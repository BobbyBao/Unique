...
{
    Unique::Swap(length_, str.length_);
    Unique::Swap(capacity_, str.capacity_);
    Unique::Swap(buffer_, str.buffer_);
}
...
String& String::AppendWithFormatArgs(const char* formatString, va_list args)
{
    ...
    while (true)
    {
        ...
        default:
            UNIQUE_LOGWARNINGF("Unsupported format specifier: '%c'", format);
            break;
        }
    }
}
